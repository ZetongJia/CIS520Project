...
#         images_batch, labels_batch = load_data(file = "data_batch_" + str(i+5))
...
import operator 
with open('COLORFeatureVecs5.txt', 'w') as f:
    for i in range(238809):
        print(i)
        fileName = 'segmentedImage3/' + str(i+1) + '.png'
        img = cv2.imread(fileName);
        #img = np.uint8(255 * images_train[i]);
        chans = cv2.split(img)
        colors = ("b", "g", "r")
        features = []
        ...
        for (chan, color) in zip(chans, colors):
          ...
          hist = reduce(operator.add, hist.tolist());
